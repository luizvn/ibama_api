    ...
    def extract_zip(self, zip_path: str) -> List[str]:
        try:
            ...
            with zipfile.ZipFile(zip_path, 'r') as zip_ref:
                ...
                logger.info(f"Encontrados {len(csv_files_in_zip)} arquivos CSV: {csv_files_in_zip}")
                ...
                for csv_name in csv_files_in_zip:
                    ...
                    with zip_ref.open(csv_name) as extracted_csv_stream:
                        ...
                        with tempfile.NamedTemporaryFile(suffix=".csv", delete=False) as temp_csv_file:
                            ...
                            logger.debug(f"Arquivo {csv_name} extraído para {temp_csv_file.name}")
            
            logger.info(f"Extração concluída. Total de CSVs prontos: {len(extracted_csv_paths)}")
    ...
    async def fetch_and_extract_all_csvs(self) -> Optional[List[str]]:
        try:
            ...
            async with aiofiles.open(temp_zip_file_path, 'wb') as temp_zip_file:
                ...
                logger.info(f"Download do ZIP concluído. Bytes: {os.path.getsize(temp_zip_file_path)}")
