...
try:
    ...
    print("Erro Crítico: Não foi possível importar os módulos da 'app'.", file=sys.stderr)
...
async def run_etl_pipeline():
    ...
    try:
        ...
        logger.info(f"Crawler concluído. {len(csv_path_list)} arquivos prontos para ingestão.")
        ...
        for csv_path in csv_path_list:
            try:
                ...
                logger.error(f"[Ingestion] FALHA ao processar o arquivo {csv_path}: {e}", exc_info=True)
                files_failed += 1
        
        logger.info("--- PIPELINE DE ETL CONCLUÍDO ---")
        logger.info(f"Resumo: {files_processed} arquivos processados, {files_failed} falharam.")
